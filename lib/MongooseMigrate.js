...
var defaultConfig = {
    ...
    db: 'mongoose-migrate',
    port: 27017,
    migrations: './**/*.migration.js,!./node_modules/**',
    template: path.resolve(__dirname, 'MigrationTemplate.js'),
...
exports.create = function (name, config, done) {
    ...
    logger.debug(config, process.cwd());
    ...
    multiGlob.glob(paths, function (err, results) {
        //filter out negated files
        files = grunt.file.match(paths, results);
        logger.info(files);
        filesLoadedFunction();
    });
    ...
    var filesLoadedFunction = function () {
        ...
        var output = path.resolve(config.templateOutputDir, latest + name);
...
function MongooseMigration(mongoose, config, done) {
    ...
    function drop(done) {
        logger.warn('Dropping database!!', config.db);
    ...
    function dropDatabase(done){
        connection.connection.db.dropDatabase(function (err, result) {
            logger.debug('Dropped Database', config.db, ' Error:', err, 'Result:', result);
            if(_.isFunction(done)){
                done(err, result);
            }
        });
